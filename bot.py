...
def init_sheets(retries=5, delay=2):
    """Инициализация spreadsheet/sheet_tasks/sheet_status с ретраями.
    Возвращает True/False, исключения не выбрасывает."""
    global spreadsheet, sheet_tasks, sheet_status
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if st and ss:
                ...
                print(f"⚠️ Не найдены листы с A1='Дата' и 'Водитель' (попытка {attempt}/{retries})")
...
def register_name(message):
    ...
    # гарантируем готовность листов (если квота/сеть — подождём)
    if sheet_status is None or sheet_tasks is None:
        init_sheets()
