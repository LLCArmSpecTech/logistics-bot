...
def fallback_handler(message):
    ...
    if users.get(chat_id, {}).get('waiting', False):
        return  # Ждём от пользователя ввод — не мешаем

    # Игнорируем тех, кто ещё не зарегистрирован через код
...
def process_task_choice(message):
    ...
    if message.text == "✅ Взять задание":
        ...
        markup = types.ReplyKeyboardRemove()
        users[chat_id]['waiting'] = True
        msg = bot.send_message(chat_id, "Вы успешно приняли задание. Сколько времени нужно на выполнение?", reply_markup=markup)
        bot.register_next_step_handler_by_chat_id(chat_id, save_eta)
