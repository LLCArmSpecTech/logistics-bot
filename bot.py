            ...
            for chat_id, data in users.items():
                if not data.get('waiting'):
                    ...
                    for col_idx, label in [(11, 'Доп. задание 1'), (13, 'Доп. задание 2'), (15, 'Доп. задание 3')]:
                        ...
                        if (
                            task and
                            not sheet_tasks.cell(row, col_idx + 1).value and
                            (row, col_idx) not in sent_additional_tasks[chat_id]
                        ):
                            ...
                            sent_additional_tasks[chat_id].add((row, col_idx))  # ✅ чтобы не присылать повторно
        except Exception as e:
            print("Ошибка в мониторинге доп. заданий:", e)
        time.sleep(10)  # также стоит увеличить интервал, чтобы не превышать лимит Google API
