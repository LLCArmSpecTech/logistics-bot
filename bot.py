...
def verify_invite_code(message):
    ...
    if code in valid_invite_codes:
        users[chat_id] = {"authorized": True}  # <-- Сохраняем флаг
...
def fallback_handler(message):
    ...
    if any(word in text for word in BAD_KEYWORDS) or len(text) > MAX_MESSAGE_LENGTH:
        bot.send_message(chat_id, "🚫 Сообщение заблокировано как подозрительное.")
        return

    # Игнорируем неизвестных пользователей
    if chat_id not in users:
        bot.send_message(chat_id, "⛔ У вас нет доступа к этому боту.")
        return

    # Если ожидается ввод (например, время или комментарий) — не мешаем
    if users.get(chat_id, {}).get('waiting', True):
        return

    # Если активна задача — показать кнопки действий
...
def process_task_choice(message):
    ...
    if message.text == "✅ Взять задание":
        ...
        markup = types.ReplyKeyboardRemove()
        bot.send_message(chat_id, "Вы успешно приняли задание. Сколько времени нужно на выполнение?", reply_markup=markup)
...
def monitor_additional():
    while True:
        try:
            for chat_id, data in users.items():
                # Проверка: водитель должен быть не в режиме ожидания и иметь текущее задание
                if not data.get('waiting') and 'current_row' in data and data['current_row'] is not None:
                    ...
                    for col_idx, label in [(11, 'Доп. задание 1'), (13, 'Доп. задание 2'), (15, 'Доп. задание 3')]:
                        ...
                        # Проверка: если задание есть, не принято и не отправлялось ранее
...
def fallback_handler(message):
    ...
    # Игнорируем сообщения, если пользователь не зарегистрирован
    if chat_id not in users:
        return
    ...
    # ✅ Если у пользователя есть активная задача — вернуть кнопки действий
    if 'current_row' in users[chat_id]:
        show_task_actions(chat_id)
        return

    # Удаляем клавиатуру и не реагируем
    markup = types.ReplyKeyboardRemove()
    bot.send_message(chat_id, "Пожалуйста, дождитесь следующего задания.", reply_markup=markup)
