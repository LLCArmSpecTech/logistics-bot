...
def check_and_send_task(chat_id, name):
    ...
    all_rows = sheet_tasks.get_all_values()[1:]  # Пропускаем строку заголовков

    for i, row in enumerate(all_rows, start=2):  # Старт с 2, потому что строку 1 пропустили
        if len(row) < 11:
            continue
        дата = row[0].strip()
        водитель = row[1].strip()
        задание = row[2].strip()
        статус = row[10].strip()  # Столбец K

        if дата == today and водитель == name and статус == "":
            msg = f"📋 Основное задание:\n{задание}"
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("✅ Принять", callback_data=f"accept_main_{i}"))
            markup.add(types.InlineKeyboardButton("❌ Невозможно", callback_data=f"reject_main_{i}"))
            bot.send_message(chat_id, msg, reply_markup=markup)
            return  # Отправлено — выходим
