...
def register_name(message):
    ...
    check_and_send_main_task(chat_id, name)
...
def check_and_send_task(chat_id, name):
    ...
    for idx, row in enumerate(tasks, start=2):
        if row['Дата'] == today and row['Водитель'] == name and not row['Статус']:
            ...
            return  # ← важно выйти из цикла после отправки
...
def check_and_send_main_task(chat_id, name):
    import datetime
    today = datetime.datetime.now().strftime('%Y-%m-%d')
    data = sheet_tasks.get_all_values()
    headers = data[1]  # строка с заголовками
    for i, row in enumerate(data[2:], start=3):  # начинаем со строки 3
        row_dict = dict(zip(headers, row))

        task_date = row_dict.get('Дата', '').strip()
        driver = row_dict.get('Водитель', '').strip()
        status = row_dict.get('Статус', '').strip()
        task = row_dict.get('Задача', '').strip()

        if task_date == today and driver == name and status == '':
            users[chat_id]['current_row'] = i
            users[chat_id]['waiting'] = False
            sent_tasks[chat_id].add(i)

msg = f"📋 Основное задание: msg ="
{task}"
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            markup.add("✅ Взять задание", "❌ Невозможно взять задание")
