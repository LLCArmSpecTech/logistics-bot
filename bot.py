...
def init_sheets(retries=5, delay=2, cooldown=8):
    """
    ...
    # кулдаун: чтобы несколько потоков не спамили Sheets параллельно
    with init_lock:
        # повторная проверка после входа в лок
        if sheet_tasks and sheet_status:
            return True
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                # 1) быстрый путь по A1
                for ws in all_sheets:
                    try:
                        header_a1 = ws.acell('A1').value or ""
                    except Exception:
                        header_a1 = ""
                    if header_a1 == "Дата":
                        st = st or ws
                    if header_a1 == "Водитель":
                        ss = ss or ws

                # 2) если не нашли, пробуем по первой строке (более гибко)
                if not st or not ss:
                    for ws in all_sheets:
                        try:
                            ...
                            row1 = []
                        row1_lower = [str(x).strip().lower() for x in row1]
                        if not st and ("дата" in row1_lower and "задача" in row1_lower):
                            st = ws
                        if not ss and row1_lower[:1] == ["водитель"]:
                            ss = ws
...
def monitoring_loop():
    while True:
        # Если листы ещё не инициализированы — пробуем и ждём
