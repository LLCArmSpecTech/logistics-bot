...
def fallback_handler(message):
    ...
    # 1. Если ожидаем ввод — обрабатываем вручную
    if users.get(chat_id, {}).get('waiting', False):
        next_step = users[chat_id].get('next')
        if next_step == 'save_eta':
            users[chat_id]['waiting'] = False
            users[chat_id]['next'] = None
            return save_eta(message)
        return  # ждём другой ввод (например, комментарий)

    # 2. Если не авторизован — игнор
    if not users.get(chat_id, {}).get('authorized', False):
        return

    # 3. Если есть активная задача — показать кнопки
...
def process_task_choice(message):
    ...
    if message.text == "✅ Взять задание":
        ...
        bot.send_message(chat_id, "⏱ Введите время, которое Вам нужно для выполнения задачи:", reply_markup=markup)
