...
def fallback_handler(message):
    ...
    # Только если есть активная задача, но не ждём конкретного ввода
    if 'current_row' in users.get(chat_id, {}) and not users[chat_id].get('waiting'):
...
@bot.message_handler(func=lambda m: users.get(m.chat.id, {}).get('waiting') and users[m.chat.id].get('next') == 'save_eta')
def wait_for_eta(message):
    users[message.chat.id]['waiting'] = False
    users[message.chat.id]['next'] = None
    save_eta(message)
...
def ask_eta(chat_id):
    ...
    bot.send_message(chat_id, "⏱ Введите время, которое Вам нужно для выполнения задачи:")
