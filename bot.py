...
def check_and_send_task(chat_id, name):
    ...
    for idx, row in enumerate(tasks, start=2):
        if row['Дата'] == today and row['Водитель'] == name and not str(row['Статус']).strip():
            task_id = idx
...
def handle_additional_task_response(message):
    ...
    result_col = 13 + (task_number - 1) * 2
    comment_col = result_col - 1
    ...
    if message.text == "✅ Принять доп. задание":
        ...
        # Возвращаем кнопки "выполнено", "невозможно", "комментарий"
...
def handle_additional_task_response(message):
    ...
    if message.text == "✅ Принять доп. задание":
        ...
        # Возвращаем обычные кнопки: "выполнено", "невозможно", "комментарий"
        show_task_actions(chat_id)
...
def handle_additional_task_decline_reason(message):
    ...
    row = additional_task_pending[chat_id]['row']
    task_number = additional_task_pending[chat_id]['task_number']
    result_col = 13 + (task_number - 1) * 2  # N=14, P=16
    comment_col = result_col - 1             # M=13, O=15
    now_time = (datetime.datetime.utcnow() + datetime.timedelta(hours=3)).strftime("%H:%M")
    reason = message.text.strip()

    sheet_tasks.update_cell(row, comment_col, f"{now_time} — {reason}")
    sheet_tasks.update_cell(row, result_col, f"Отказ от доп. задания {task_number}")

    bot.send_message(chat_id, f"Отказ от доп. задания {task_number} сохранён.")
    del additional_task_pending[chat_id]

    # Возвращаем кнопки для основного задания
    show_task_actions(chat_id)
